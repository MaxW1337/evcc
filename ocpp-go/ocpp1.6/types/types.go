...
// UnmarshalJSON implements json.Unmarshaler to tolerantly accept limit values
// encoded as either a JSON number or a JSON string (non-conformant wallboxes).
func (p *ChargingSchedulePeriod) UnmarshalJSON(data []byte) error {
	...
	// Try number first
	var f float64
	if err := json.Unmarshal(aux.Limit, &f); err == nil {
		p.Limit = f
		return nil
	}
	// Fall back to quoted string (e.g. "14.0")
	var s string
	if err := json.Unmarshal(aux.Limit, &s); err != nil {
		return err
	}
